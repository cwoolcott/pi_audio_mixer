...
function audioTest(option) {
  ...
  if (option === 'Get Audio Device') {
    ...
    inquirer
      ...
      .then(answers => {
        let soundCards = cp.execSync("amixer set " + answers.channelName + " + 50%");
    ...
    inquirer
      ...
      .then(answers => {
        let soundCards = cp.execSync("amixer set " + answers.channelName + " + 100%");
        console.table(soundCards);
      })
    ...
    .prompt([
      {
        ...
        name: 'option',
        message: 'TESTING:',
        choices: ['controls', '50% Volume', '100% Volume', 'Test', 'Exit'],
      },
    ])
    .then(answers => {
